...
=== Zammad Installer - v.0.001 (Debian) ===
...
apt-get install apt-transport-https wget locales sudo gnupg2 firewalld -y
...
echo -e "== Installing prerequisites..."
apt-get install apt-transport-https wget locales sudo firewalld -y
...
wget -O /etc/apt/sources.list.d/zammad.list https://dl.packager.io/srv/zammad/zammad/stable/installer/debian/${debian_version}.repo
checkStatus
